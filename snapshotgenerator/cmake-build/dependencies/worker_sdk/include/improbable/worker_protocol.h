...
#if defined(CORE_SDK_DLL) && (defined(_MSC_VER) || defined(__ORBIS__))
...
#else /* defined(CORE_SDK_DLL) && (defined(_MSC_VER) || defined(__ORBIS__)) */
#define CORE_SDK_API
#endif /* defined(CORE_SDK_DLL) && (defined(_MSC_VER) || defined(__ORBIS__)) */
...
/** Enum used to control the binary logger. */
typedef enum WorkerProtocol_BinaryLoggerType {
  WORKER_PROTOCOL_BINARY_LOGGER_TYPE_NONE = 0,
  WORKER_PROTOCOL_BINARY_LOGGER_TYPE_FILE = 1,
  WORKER_PROTOCOL_BINARY_LOGGER_TYPE_TRIGGERED = 2,
} WorkerProtocol_BinaryLoggerType;
...
typedef void WorkerProtocol_ClientFree(WorkerProtocol_ComponentId component_id, void* user_data,
                                       uint8_t object_type, WorkerProtocol_ClientHandle* handle);
/** Component vtable typedef. */
typedef WorkerProtocol_ClientHandle*
WorkerProtocol_ClientCopy(WorkerProtocol_ComponentId component_id, void* user_data,
                          uint8_t object_type, const WorkerProtocol_ClientHandle* handle);
/** Component vtable typedef. */
typedef uint8_t WorkerProtocol_ClientDeserialize(WorkerProtocol_ComponentId component_id,
                                                 void* user_data, uint8_t object_type,
                                                 Pbio_Object* source,
 ...
 * the connection for each component in WorkerProtocol_ConnectionParameters, unless a default vtable
 * is specified. This allows the API to perform component update serialization and similar tasks in
 * a target-language agnostic way.
 *
...
typedef struct WorkerProtocol_ComponentVtable {
  /**
   * Component ID that this vtable is for. If this is the default vtable, this field should be set
   * to 0.
   */
...
typedef struct WorkerProtocol_EntityQuery {
  ...
  const WorkerProtocol_ComponentId* SnapshotResultTypeComponentId;
...
typedef enum WorkerProtocol_OpType {
  WORKER_PROTOCOL_OP_TYPE_DISCONNECT = 1,
  WORKER_PROTOCOL_OP_TYPE_FLAG_UPDATE = 2,
  WORKER_PROTOCOL_OP_TYPE_LOG_MESSAGE = 3,
  WORKER_PROTOCOL_OP_TYPE_METRICS = 4,
  WORKER_PROTOCOL_OP_TYPE_CRITICAL_SECTION = 5,
  WORKER_PROTOCOL_OP_TYPE_ADD_ENTITY = 6,
  WORKER_PROTOCOL_OP_TYPE_REMOVE_ENTITY = 7,
  WORKER_PROTOCOL_OP_TYPE_RESERVE_ENTITY_ID_RESPONSE = 8,
  WORKER_PROTOCOL_OP_TYPE_RESERVE_ENTITY_IDS_RESPONSE = 9,
  WORKER_PROTOCOL_OP_TYPE_CREATE_ENTITY_RESPONSE = 10,
  WORKER_PROTOCOL_OP_TYPE_DELETE_ENTITY_RESPONSE = 11,
  WORKER_PROTOCOL_OP_TYPE_ENTITY_QUERY_RESPONSE = 12,
  WORKER_PROTOCOL_OP_TYPE_ADD_COMPONENT = 13,
  WORKER_PROTOCOL_OP_TYPE_REMOVE_COMPONENT = 14,
  WORKER_PROTOCOL_OP_TYPE_AUTHORITY_CHANGE = 15,
  WORKER_PROTOCOL_OP_TYPE_COMPONENT_UPDATE = 16,
  WORKER_PROTOCOL_OP_TYPE_COMMAND_REQUEST = 17,
  WORKER_PROTOCOL_OP_TYPE_COMMAND_RESPONSE = 18,
} WorkerProtocol_OpType;

/** Data for a single op contained within an op list. */
typedef struct WorkerProtocol_Op {
  /** The type of this op, defined in WorkerProtocol_OpType. */
  uint8_t OpType;
  union {
    WorkerProtocol_DisconnectOp Disconnect;
    WorkerProtocol_FlagUpdateOp FlagUpdate;
    WorkerProtocol_LogMessageOp LogMessage;
    WorkerProtocol_MetricsOp Metrics;
    WorkerProtocol_CriticalSectionOp CriticalSection;
    WorkerProtocol_AddEntityOp AddEntity;
    WorkerProtocol_RemoveEntityOp RemoveEntity;
    WorkerProtocol_ReserveEntityIdResponseOp ReserveEntityIdResponse;
    WorkerProtocol_ReserveEntityIdsResponseOp ReserveEntityIdsResponse;
    WorkerProtocol_CreateEntityResponseOp CreateEntityResponse;
    WorkerProtocol_DeleteEntityResponseOp DeleteEntityResponse;
    WorkerProtocol_EntityQueryResponseOp EntityQueryResponse;
    WorkerProtocol_AddComponentOp AddComponent;
    WorkerProtocol_RemoveComponentOp RemoveComponent;
    WorkerProtocol_AuthorityChangeOp AuthorityChange;
    WorkerProtocol_ComponentUpdateOp ComponentUpdate;
    WorkerProtocol_CommandRequestOp CommandRequest;
    WorkerProtocol_CommandResponseOp CommandResponse;
  };
} WorkerProtocol_Op;

/** An op list, usually returned by WorkerProtocol_Connection_GetOpList. */
typedef struct WorkerProtocol_OpList {
  WorkerProtocol_Op* Op;
  uint32_t OpCount;
} WorkerProtocol_OpList;
...
/** Parameters for BinaryFileLogger. */
typedef struct WorkerProtocol_BinaryFileLoggerParameters {
  /** Relative or absolute path of the file to which logs are going to be written. */
  const char* Filename;
  /**
   * If the next message to be logged would make the file size in bytes greater
   * than MaxFileSizeBytes logging stops without writing that message to disk.
   */
  uint64_t MaxFileSizeBytes;
} WorkerProtocol_BinaryFileLoggerParameters;

/**
 * Parameters for BinaryTriggeredLogger.
 * If WorkerProtocol_Connection_RecordBinaryLogEvent() is called
 * BinaryTriggeredLogger writes to disk all logs that happened and will happen in the interval:
 *
 * [WorkerProtocol_Connection_RecordBinaryLogEvent() call - MillisecondsBefore,
 *  WorkerProtocol_Connection_RecordBinaryLogEvent() call + MillisecondsAfter]
 *
 * in the order they were logged.
 *
 * In case of overlapping intervals logs from their union is written to a single file.
 */
typedef struct WorkerProtocol_BinaryTriggeredLoggerParameters {
  /**
   * File names created by this logsink are prefixed with this prefix, are numbered,
   * and have the extension .log. The prefix can be any relative or absolute path.
   */
  const char* FilenamePrefix;
  /**
   * How long should logs be kept in memory in case
   * WorkerProtocol_Connection_RecordBinaryLogEvent() is called.
   */
  uint64_t MillisecondsBefore;
  /**
   * How long after WorkerProtocol_Connection_RecordBinaryLogEvent() call
   * should logs be written to disk.
   */
  uint64_t MillisecondsAfter;
} WorkerProtocol_BinaryTriggeredLoggerParameters;

/**
 * Parameters for configuring binary logging.
 */
typedef struct WorkerProtocol_BinaryLoggerParameters {
  /**
   * If Type != WORKER_PROTOCOL_BINARY_LOGGER_TYPE_NONE binary logging is enabled with
   * the appropriate parameters.
   */
  uint8_t Type;
  /** Used if Type == WORKER_PROTOCOL_BINARY_LOGGER_TYPE_FILE. */
  WorkerProtocol_BinaryFileLoggerParameters BinaryFile;
  /** Used if Type == WORKER_PROTOCOL_BINARY_LOGGER_TYPE_TRIGGERED. */
  WorkerProtocol_BinaryTriggeredLoggerParameters BinaryTriggered;
} WorkerProtocol_BinaryLoggerParameters;
 ...
 * will have no thread affinity. Used by WorkerProtocol_SdkParameters.
 * */
typedef struct WorkerProtocol_ThreadAffinityParameters {
  /**
   * Affinity mask for threads related to receiving network ops, see documentation for specifics.
   */
  uint64_t ReceiveThreadsAffinityMask;
  /** Affinity mask for threads related to sending network ops, see documentation for specifics. */
  uint64_t SendThreadsAffinityMask;
  /** Affinity mask for short-lived threads. */
  uint64_t TemporaryThreadsAffinityMask;
} WorkerProtocol_ThreadAffinityParameters;
...
typedef struct WorkerProtocol_SdkParameters {
  ...
  /**
   * Parameters for configuring binary logging. The decision to enable or disable the binary
   * logger can only be made at startup and cannot be changed later on.
   */
  WorkerProtocol_BinaryLoggerParameters BinaryLoggerParameters;

  /** Parameters for configuring thread affinity. Used by WorkerProtocol_SdkParameters. */
  WorkerProtocol_ThreadAffinityParameters ThreadAffinity;
...
typedef struct WorkerProtocol_PlayerIdentityCredentials {
  const char* PlayerIdentityToken;
  const char* LoginToken;
} WorkerProtocol_PlayerIdentityCredentials;
...
typedef enum WorkerProtocol_LocatorFlags {
  ...
  WORKER_PROTOCOL_LOCATOR_PLAYER_IDENTITY_CREDENTIALS = 4,
...
typedef struct WorkerProtocol_LocatorParameters {
  ...
  /** Parameters used if the WORKER_PROTOCOL_PLAYER_IDENTITY_TOKEN_CREDENTIALS flag is set. */
  WorkerProtocol_PlayerIdentityCredentials PlayerIdentity;

  /** Whether to use an insecure (non-TLS) connection for local development. */
  uint8_t UseInsecureConnection;
...
/**
 * Connects to a specific deployment using a player identity token and login token. The deployment
 * and project identifiers are encoded into the tokens; a PIT and login token should be obtained
 * from a game authentication server or the Simple Auth Service and provided when the
 * WorkerProtocol_Locator is created.
 *
 * Returns a WorkerProtocol_ConnectionFuture that can be used to obtain a WorkerProtocol_Connection
 * by using WorkerProtocol_ConnectionFuture_Get. Caller is responsible for destroying it when no
 * longer needed by using WorkerProtocol_ConnectionFuture_Destroy.
 */
CORE_SDK_API WorkerProtocol_ConnectionFuture*
WorkerProtocol_LocatorV2_ConnectAsync(const WorkerProtocol_Locator* locator,
...
/**
 * Connect directly to an already running bridge in a SpatialOS deployment whose hostname and port
 * are known.
 *
 * Returns a WorkerProtocol_ConnectionFuture that can be used to obtain a WorkerProtocol_Connection
 * by using WorkerProtocol_ConnectionFuture_Get. Caller is responsible for destroying it when no
 ...
 */
CORE_SDK_API WorkerProtocol_ConnectionFuture*
WorkerProtocol_ConnectDirectAsync(const char* bridge_hostname, uint16_t bridge_port,
                                  const char* bridge_session_token,
                                  const WorkerProtocol_ConnectionParameters* params);
...
WorkerProtocol_Connection_SetProtocolLoggingEnabled(WorkerProtocol_Connection* connection,
                                                    uint8_t enabled);
/**
 * If binary logging is enabled logs a special event tag and propagates the
 * information that an event has happened to the binary logger.
 *
 * If using the BinaryTriggeredLogger this causes logs to be written to disk.
 */
CORE_SDK_API void
WorkerProtocol_Connection_RecordBinaryLogEvent(WorkerProtocol_Connection* connection);

/**
 * If binary logging is enabled this will log a message that corresponds to the
 * last WorkerProtocol_Connection_RecordEvent() call. If message is empty doesn't do anything.
 * Can only be called after WorkerProtocol_Connection_RecordEvent() has been called at least once.
 */
CORE_SDK_API void
WorkerProtocol_Connection_RecordBinaryLogEventMessage(WorkerProtocol_Connection* connection,
                                                      const char* message);

...
enum class BinaryLoggingType { kNone = 0, kFile = 1, kTriggered = 2 };

/** Parameters for BinaryFileLogger. */
struct BinaryFileLoggerParameters {
 ...
 * If alpha::RecordBinaryLogEvent() is called
 * BinaryTriggeredLogger writes to disk all logs that happened and will happen in the interval:
 *
 * [alpha::RecordBinaryLogEvent() call - MillisecondsBefore,
 *  alpha::RecordBinaryLogEvent() call + MillisecondsAfter]
 *
 * in the order they were logged.
 *
 * In case of overlapping intervals logs from their union is written to a single file.
 */
struct BinaryTriggeredLoggerParameters {
  /**
   * File names created by this logger are prefixed with this prefix, are numbered,
   * and have the extension .log. The prefix can be any relative or absolute path.
   */
  std::string FilenamePrefix;
  /** How long should logs be kept in memory in case alpha::RecordEvent() is called. */
  std::uint64_t MillisecondsBefore = 0;
  /** How long after alpha::RecordEvent() call should logs be written to disk. */
  std::uint64_t MillisecondsAfter = 0;
};

/**
 * Parameters for configuring binary logging.
 */
struct BinaryLoggingParameters {
  /**
   * If Type != BinaryLoggingType::kNone binary logging is enabled with
   * the appropriate parameters.
   */
  BinaryLoggingType Type = BinaryLoggingType::kNone;
  /** Used if Type == BinaryLoggingType::kFile. */
  BinaryFileLoggerParameters BinaryFile;
  /** Used if Type == BinaryLoggingType::kTriggered. */
  BinaryTriggeredLoggerParameters BinaryTriggered;
};
 ...
 *
 * Specific action in the binary logger depends on the logger that is active.
 */
inline void RecordBinaryLogEvent(const Connection& connection);
/**
 * If binary logging is enabled this will log a message that will correspond to the
 * last alpha::RecordBinaryLogEvent(). If message is empty doesn't do anything.
 * Can only be called after alpha::RecordBinaryLogEvent() has been called at least once.
 */
...
struct NetworkParameters {
  ...
  std::uint64_t ConnectionTimeoutMillis = defaults::kConnectionTimeoutMillis;
  /** Default timeout for worker commands if one is not specified when command is sent. */
  std::uint32_t DefaultCommandTimeoutMillis = defaults::kDefaultCommandTimeoutMillis;
};
...
/**
 * Parameters for configuring SpatialOS thread affinities on Windows or console. This does NOT
 * work on MacOS or Linux. Used by WorkerProtocol_SdkParameters. Affinity masks are bit masks
 * where having 1 in the nth least significant position means the thread will be permitted to
 * run on the nth core. If an affinity mask is set to zero, the group of threads using that
 * mask will have no thread affinity.
 * */
struct ThreadAffinityParameters {
  /**
   * Affinity mask for threads related to receiving network ops, see documentation for
   * specifics.
   */
  uint64_t ReceiveThreadsAffinityMask = 0;
  /** Affinity mask for threads related to sending network ops, see documentation for specifics. */
  uint64_t SendThreadsAffinityMask = 0;
  /** Affinity mask for short-lived threads. */
  uint64_t TemporaryThreadsAffinityMask = 0;
};
...
struct ConnectionParameters {
   ...
   */
  alpha::BinaryLoggingParameters BinaryLogging;

  /** Parameters for configuring thread affinity. */
  ThreadAffinityParameters ThreadAffinity;
};

/** Overrides the default interest settings for a particular entity and component. */
struct InterestOverride {
  InterestOverride(bool is_interested = false) : IsInterested{is_interested} {}
...
struct LocatorParameters {
  ...
  ProtocolLoggingParameters Logging;
...
struct CommandParameters {
  CommandParameters(bool allow_short_circuit = false) : AllowShortCircuit{allow_short_circuit} {}
  /**
...
struct PlayerIdentityCredentials {
  /**
...
/**
 * A client which can be used to connect to a SpatialOS deployment via the locator service.
...
public:
   ...
   * provided at construction in the LocatorParameters, along with a player identity token.
...
public:
  ...
  static Future<Connection> ConnectAsync(const ComponentRegistry& registry,
                                         const std::string& receptionist_hostname,
                                         std::uint16_t receptionist_port,
...
private:
  ...
  friend class alpha::Locator;
  friend class TestConnection;
  friend inline void alpha::RecordBinaryLogEvent(const Connection& connection);
  friend inline void alpha::RecordBinaryLogEventMessage(const Connection& connection,
                                                        const std::string& message);
...
#include <improbable/detail/alpha.i.h>
